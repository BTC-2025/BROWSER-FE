...
export default function HistoryBookmarksPage() {
    ...
    return (
        <div className="flex-1 flex flex-col overflow-hidden relative" style={{ background: '#101622' }}>
            {/* Background Glows */}
            <div className="absolute top-[-20%] left-[-10%] w-[500px] h-[500px] bg-[#135bec]/20 rounded-full blur-[120px] pointer-events-none opacity-40" />
            ...
            {/* Top Bar */}
            <header className="flex items-center justify-between px-6 py-3 border-b border-slate-800 bg-[#101622]/80 backdrop-blur-md z-50">
                <div className="flex items-center gap-4">
                    <div className="size-8 text-[#135bec] flex items-center justify-center bg-[#135bec]/10 rounded-lg">
                        <span className="material-symbols-outlined text-2xl">public</span>
                    </div>
                    <h2 className="text-lg font-bold leading-tight tracking-tight text-white">Nexus Browser</h2>
                </div>
                <nav className="hidden md:flex items-center gap-8">
                    <a className="text-slate-400 hover:text-[#135bec] transition-colors text-sm font-medium cursor-pointer">Dashboard</a>
                    <a className="text-[#135bec] font-bold text-sm cursor-pointer">Library</a>
                    <a className="text-slate-400 hover:text-[#135bec] transition-colors text-sm font-medium cursor-pointer">Extensions</a>
                </nav>
            </header>
                ...
                <aside className="flex flex-col w-full md:w-[400px] lg:w-[450px] shrink-0 glass-panel rounded-2xl overflow-hidden shadow-xl shadow-black/20">
                    ...
                    <div className="p-5 border-b border-white/5 bg-white/5 backdrop-blur-sm">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <button className="text-xs font-medium text-slate-500 hover:text-red-400 transition-colors px-3 py-1.5 rounded-full hover:bg-white/5">
                                Clear Data
                            </button>
                        </div>
                        {/* Search */}
                        <div className="relative group">
                            <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-[#135bec] transition-colors text-xl">search</span>
                            <input
                                className="w-full bg-slate-900/50 border border-slate-700 rounded-xl py-2.5 pl-10 pr-4 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:border-[#135bec] focus:ring-1 focus:ring-[#135bec] transition-all"
                    ...
                    <div className="flex-1 overflow-y-auto p-2 space-y-6">
                        {Object.entries(historyData).map(([group, items]) => (
                            <div key={group} className="px-3">
                                <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3 ml-2">{group}</h4>
                                    ...
                                    {items
                                        ...
                                        .map((item) => (
                                            <a key={item.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-white/5 group transition-colors relative cursor-pointer">
                                                <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-[#135bec] rounded-r-full opacity-0 group-hover:opacity-100 transition-opacity" />
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-sm font-medium text-slate-200 truncate group-hover:text-[#135bec] transition-colors">{item.title}</p>
                ...
                <section className="flex-1 flex flex-col glass-panel rounded-2xl overflow-hidden shadow-xl shadow-black/20">
                    ...
                    <div className="p-6 border-b border-white/5 bg-white/5 backdrop-blur-sm flex flex-col gap-4">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                <span className="material-symbols-outlined text-[#135bec]">bookmarks</span>
                                <h3 className="text-xl font-bold text-white">Bookmarks</h3>
                            </div>
                            <button className="flex items-center gap-2 px-4 py-2 bg-[#135bec] hover:bg-blue-600 text-white text-sm font-semibold rounded-lg transition-all shadow-lg shadow-[#135bec]/20">
                                ...
                                <div key={bm.id} className="glass-card group rounded-xl p-4 flex flex-col gap-3 relative cursor-pointer">
                                    ...
                                    <div className="h-32 rounded-lg bg-slate-800 w-full overflow-hidden border border-slate-700 relative">
                                        <div className={`w-full h-full bg-gradient-to-tr ${bm.gradient} flex items-center justify-center`}>
                                            <span className="material-symbols-outlined text-4xl text-white/20">{bm.icon}</span>
                                        </div>
                                    </div>
                                    <div>
                                        <h4 className="font-semibold text-slate-200 truncate pr-6 group-hover:text-[#135bec] transition-colors">{bm.title}</h4>
                                        <p className="text-xs text-slate-500 truncate">{bm.url}</p>
                                    </div>
                                </div>
                            ))}
                            {/* Add Bookmark Card */}
                            <div className="rounded-xl p-4 flex flex-col items-center justify-center gap-3 cursor-pointer border border-dashed border-slate-700 hover:border-[#135bec]/50 hover:bg-[#135bec]/5 transition-colors group">
                                <div className="size-12 rounded-full bg-slate-800 group-hover:bg-[#135bec]/20 flex items-center justify-center transition-colors">
                                    <span className="material-symbols-outlined text-slate-400 group-hover:text-[#135bec]">add</span>
                                </div>
                                <p className="text-sm font-medium text-slate-500 group-hover:text-[#135bec] transition-colors">Add Bookmark</p>
                            </div>
                        </div>
                    </div>
                </section>
