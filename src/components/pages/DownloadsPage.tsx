...
const sampleDownloads = [
    ...
    { id: '3', name: 'dive-browser-v128.dmg', size: '195 MB', status: 'downloading', progress: 67, icon: 'desktop_windows', date: 'Now' },
...
export default function DownloadsPage() {
    ...
    return (
        <div className="flex-1 flex flex-col overflow-hidden relative" style={{ background: '#FFFFFF' }}>
            {/* Background Glows */}
            <div className="absolute top-[-20%] left-[20%] w-[500px] h-[500px] bg-[#004AAD]/15 rounded-full blur-[120px] pointer-events-none" />
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex flex-col gap-2">
                            <div className="flex items-center gap-3">
                                <div className="p-2 rounded-xl bg-[#004AAD]/10">
                                    <span className="material-symbols-outlined text-[#004AAD] text-2xl">download</span>
                                </div>
                                <h1 className="font-future text-3xl md:text-4xl text-[#0A1F44] tracking-tight">Downloads</h1>
                            </div>
                            <p className="text-[#5F7FA6] text-sm">Manage and track your downloaded files.</p>
                        </div>
                        <button className="text-sm font-medium text-[#5F7FA6] hover:text-red-400 px-4 py-2 rounded-lg hover:bg-white/5 transition-colors">
                            Clear All
                        </button>
                    </div>
                    ...
                    <div className="space-y-3">
                        {filteredDownloads.map((dl) => (
                            <div key={dl.id} className="glass-card rounded-xl p-4 flex items-center gap-4 group">
                                {/* Icon */}
                                <div className={`size-12 rounded-xl flex items-center justify-center shrink-0 ${dl.status === 'downloading' ? 'bg-[#004AAD]/20 text-[#004AAD]' :
                                        dl.status === 'paused' ? 'bg-amber-500/20 text-amber-400' :
                                            'bg-slate-800 text-[#5F7FA6]'
                                    }`}>
                                    <span className="material-symbols-outlined text-2xl">{dl.icon}</span>
                                </div>
                                {/* Info */}
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm font-semibold text-slate-200 truncate group-hover:text-[#004AAD] transition-colors">{dl.name}</p>
                                    <div className="flex items-center gap-3 mt-1">
                                        <span className="text-xs text-[#8FA9C9]">{dl.size}</span>
                                        <span className="text-xs text-slate-600">•</span>
                                        <span className="text-xs text-[#8FA9C9]">{dl.date}</span>
                                    </div>
                                    {/* Progress Bar */}
                                    {dl.status !== 'complete' && (
                                        <div className="mt-2 h-1.5 bg-slate-700 rounded-full overflow-hidden">
                                            <div
                                                className={`h-full rounded-full transition-all ${dl.status === 'downloading' ? 'bg-[#004AAD]' : 'bg-amber-500'}`}
                                ...
                                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    {dl.status === 'downloading' && (
                                        <button className="p-2 rounded-lg hover:bg-white/10 text-[#5F7FA6] hover:text-[#0A1F44] transition-colors">
                                    ...
                                    <button className="p-2 rounded-lg hover:bg-white/10 text-[#5F7FA6] hover:text-red-400 transition-colors">
                                        <span className="material-symbols-outlined text-[20px]">delete</span>
                                    </button>
                                </div>
                                {/* Status Badge */}
                                <div className={`px-3 py-1 rounded-full text-xs font-medium ${dl.status === 'complete' ? 'bg-green-500/20 text-green-400' :
                                        dl.status === 'downloading' ? 'bg-[#004AAD]/20 text-[#004AAD]' :
