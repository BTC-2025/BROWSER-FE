...
export default function BrowserMenu({ isOpen, onClose, triggerRef }: BrowserMenuProps) {
    ...
    const MenuItem = ({ icon, label, shortcut, onClick, danger, chevron, badge }: MenuItemProps) => (
        <button
            ...
            className={`w-full flex items-center gap-2.5 px-2.5 py-1.5 rounded-lg text-[13px] transition-colors group
                ${danger ? 'text-red-400 hover:bg-red-500/10' : 'text-slate-300 hover:bg-white/[0.07] hover:text-white'}`}
        >
            <span className={`material-symbols-outlined text-[17px] shrink-0 ${danger ? 'text-red-400' : 'text-slate-500 group-hover:text-slate-300'}`}>
                {icon}
            </span>
            <span className="flex-1 text-left">{label}</span>
            {badge && (
                <span className="text-[9px] px-1.5 py-0.5 rounded bg-[#135bec]/20 text-[#135bec] font-bold tracking-wide mr-1">
                    {badge}
                </span>
            )}
    ...
    return (
        <div
            ref={menuRef}
            className="absolute right-0 top-full mt-1 z-[200] rounded-xl border border-white/[0.08] shadow-2xl shadow-black/70 flex flex-col animate-fadeIn"
            style={{
                width: '290px',
            ...
            <div className="overflow-y-auto flex-1 py-2 px-1.5" style={{ overscrollBehavior: 'contain' }}>
                ...
                <div className="grid grid-cols-3 gap-1.5 px-1 pb-2">
                    {[
                        ...
                        { icon: 'security', label: 'Private', action: () => { openTab('nexus://private'); onClose(); } },
                    ].map((it) => (
                        <button
                            key={it.label}
                            onClick={it.action}
                            className="flex flex-col items-center gap-1 p-2 rounded-lg bg-white/[0.04] hover:bg-white/[0.09] border border-white/[0.06] text-slate-400 hover:text-white transition-all"
                        >
                ...
                <div className="flex items-center gap-1.5 px-2.5 py-1.5 mx-1 rounded-lg bg-white/[0.03] border border-white/[0.05] mb-1">
                    <span className="material-symbols-outlined text-[16px] text-slate-500">zoom_in</span>
                    <span className="text-[12px] text-slate-400 flex-1">Zoom</span>
                    <button onClick={handleZoomOut} className="w-7 h-6 flex items-center justify-center rounded hover:bg-white/10 text-slate-300 hover:text-white text-base leading-none">−</button>
                    <button onClick={handleZoomReset} className="w-12 h-6 flex items-center justify-center rounded hover:bg-white/10 text-xs font-bold text-white">{zoom}%</button>
                    <button onClick={handleZoomIn} className="w-7 h-6 flex items-center justify-center rounded hover:bg-white/10 text-slate-300 hover:text-white text-base leading-none">+</button>
                    <div className="w-px h-4 bg-white/10" />
                    <button onClick={handleFullscreen} className="w-7 h-6 flex items-center justify-center rounded hover:bg-white/10 text-slate-400 hover:text-white">
                ...
                {/* Find row (inline) */}
                {showFind && (
                    <div className="flex items-center gap-1.5 px-2.5 py-1.5 mx-1 rounded-lg bg-white/[0.03] border border-[#135bec]/30 mb-1">
                        <span className="material-symbols-outlined text-[15px] text-[#135bec]">search</span>
                        <input
                            ref={findRef}
                            type="text"
                            value={findText}
                            ...
                            className="flex-1 bg-transparent text-xs text-white placeholder-slate-600 outline-none border-none"
                        />
                        {findText && (
                            <button onClick={() => { handleFindInput(''); }} className="text-slate-500 hover:text-white">
                ...
                {/* Nav items */}
                <MenuItem icon="history" label="History" shortcut="⌘Y" onClick={() => go('nexus://history')} />
                <MenuItem icon="download" label="Downloads" shortcut="⌥⌘L" onClick={() => go('nexus://downloads')} />
                <MenuItem icon="bookmarks" label="Bookmarks" onClick={() => go('nexus://bookmarks')} chevron />
                ...
                <MenuItem icon="lock" label="Security & Privacy" onClick={() => go('nexus://security')} badge="New" />
                <MenuItem icon="settings" label="Settings" onClick={() => go('nexus://settings')} />
